...
RES PPS::FEASIBLE(Agent* c, Nodes &pi, Nodes &T, bool swap) {
...
CHECK PPS::CHECK_PUSHER(Agent* a) {
  // exclude swaper
...
Nodes PPS::SHORTEST_PATH(Agent* c, Node* g, Nodes& T) {
...
void PPS::SETUP_SWAP(Agent* c, Agent* a) {
  // error check
  if (deg3nodes.size() == 0) {
...
CHECK PPS::CHECK_PRIORITY(Agent* c, Agent* a) {
  S* sC = getS(c);
  S* sA = getS(a);
  return CHECK_PRIORITY(sC, sA);
}

CHECK PPS::CHECK_PRIORITY(S* s, Agent* a) {
  S* sA = getS(a);
