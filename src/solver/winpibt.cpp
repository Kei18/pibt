...
void winPIBT::updateGoal(Agent* a) {
  // Node* g;
...
bool winPIBT::winpibt(Agent* a, int t_tmp, bool varphi)
{
  int l = ell(a);
...
Agents::iterator winPIBT::findPITargetAgent(Node* v, int t) {
...
Node* winPIBT::getGoal(Agent* a) {
  if (a->hasGoal()) return a->getGoal();
  return lastNode(a);
}

Nodes winPIBT::getPath(Agent* a, Node* _g, int t1, int t2) {
  if (t2 <= t1 || t2 <= ell(a)) {
