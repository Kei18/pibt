...
protected:
  ...
  int ell(Agent* a);
  int ell(int i);
  Node* lastNode(Agent* a);
  Node* lastNode(int id);
  void updateGoal(Agent* a);
  Node* getGoal(Agent* a);
  Agents::iterator findPITargetAgent(Node* v, int t);
  int getTmax(int t_tmp);

  void init();
  void allocate();

  virtual bool winpibt(Agent* a, int t_tmp, bool varphi);

  Nodes getPath(Agent* a, Node* g, int t1, int t2);
